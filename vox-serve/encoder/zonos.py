...
class ZonosSpeakerEmbedding(nn.Module):
    def __init__(self, ckpt_path: str = "ResNet293_SimAM_ASP_base.pt", device: str = "cuda:0"):
        super().__init__()
        self.device = device
...
class ZonosSpeakerEmbeddingLDA(nn.Module):
    def __init__(self, device: str = "cuda:0"):
        ...
        self.device = device
        with torch.device(device):
            self.model = ZonosSpeakerEmbedding(spk_model_path, device)
