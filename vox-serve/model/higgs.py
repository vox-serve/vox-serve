...
print(ALL_ATTENTION_FUNCTIONS.keys())  # dict_keys(['eager', 'sdpa', 'flash_attention_2'])
LLAMA_ATTENTION_CLASSES = {"sdpa": LlamaAttention, "flash_attention_2": LlamaAttention} 
...
class HiggsAttention(nn.Module):
    ...
    def forward(
        ...
        # - check shape match for debugging
        if(query_states.shape != key_states.shape):
            raise RuntimeError(f"Shape mismatch: query_states shape {query_states.shape} and key_states shape {key_states.shape} do not match.")
        
        # reshape. squash to 3d
        b, s, h, d = query_states.shape
        query_states = query_states.reshape(b * s, h, d)
        key_states = key_states.reshape(b * s, h, d)
        ...
        # prepare outputs for decoder
        self_attn_weights = None  # if your wrapper doesn't give them
        present_key_value = (key_states, value_states)  # for caching

        return attn_output, self_attn_weights, present_key_value
...
class HiggsDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        # Self Attention
        _, _, hidden_states = self.self_attn(
            hidden_states=hidden_states,
            position_ids=position_ids,
...
class HiggsAudioDualFFNDecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.layer_idx = layer_idx
        self.self_attn = HiggsAttention(config=text_config, layer_idx=layer_idx)

        self.mlp = HiggsMLP(text_config)

        if not fast_forward:
            if use_audio_attention:
                self.audio_attn = HiggsAttention(config=text_config, layer_idx=layer_idx + 1)
                self.audio_post_audio_attn_layer_norm = LlamaRMSNorm(
                    text_config.hidden_size, eps=text_config.rms_norm_eps
                )

            self.audio_mlp = HiggsMLP(text_config)
...
class HiggsAudioModel(HiggsAudioPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: HiggsAudioConfig):
        ...
        if config.audio_adapter_type == "dual_ffn":
            ...
            # rebuild dual ffn model layers with fast-forward option
            for j in range(config.text_config.num_hidden_layers):
                if j in config.audio_dual_ffn_layers:
                    layers.append(
                        HiggsAudioDualFFNDecoderLayer(
            ...
            # rebuild model without dual ffn
