"""
Entry point for conductor daemon subprocess.
...
def _run_conductor_daemon(
    ...
    """Entry point for conductor daemon that sets CUDA_VISIBLE_DEVICES before importing torch."""
    # DEBUG: Check if torch is already imported (should NOT be!)
    torch_already_imported = "torch" in sys.modules
    print(
        f"[DP ENTRY] Rank {dp_rank}: Starting, torch already imported: {torch_already_imported}",
        flush=True,
    )
    print(
    ...
    print(
        f"[DP ENTRY] Rank {dp_rank}: torch imported, available devices: {torch.cuda.device_count()}",
        flush=True,
    )
    ...
    # Import conductor
    from vox_serve.conductor import load_conductor
    ...
    if dp_size > 1:
        ...
        logger.info(
            f"DP rank {dp_rank} using CUDA device {cuda_device}, torch sees {device_count} devices"
        )
    ...
    logger.info(
        f"Conductor (DP rank {dp_rank}/{dp_size}) started successfully with model: {model_name}"
    )
    conductor.run_forever()


def main():
    """Main entry point when run as a module."""
    parser = argparse.ArgumentParser(description="Vox-Serve Conductor Daemon")
    parser.add_argument("--dp-rank", type=int, required=True)
    parser.add_argument("--dp-size", type=int, required=True)
    parser.add_argument("--model-name", type=str, required=True)
    parser.add_argument("--conductor-type", type=str, required=True)
